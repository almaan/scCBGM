...
def main(methods_to_eval: list = ["cinemaot", "scgen"]):
    ...
    for cell_type in base_cell_types:
        ...
        if "cinemaot" in methods_to_eval:
            # ---- CinemaOT Baseline ----
            cinema_ot_model = CinemaOT(
                thresh=0.5, eps=0.001, concept_key="concepts", obsm_key=OBSM_KEY
            )  # not used in CinemaOT - would always work
            cinema_ot_model.train(adata_train.copy())
            pred_adata_cot = []
            for intervention in interventions:
                pred_adata_cot.append(
                    cinema_ot_model.predict_intervention(
                        adata_inter.copy(),
                        hold_out_label="dose",
        ...
        if "scgen" in methods_to_eval:
            ...
            pred_adata_scgen = []
            for intervention in interventions:
                pred_adata_scgen.append(
                    scgen_model.predict_intervention(
                        adata_inter.copy(),
                        hold_out_label="dose",
                        concepts_to_flip=[intervention["concept"]],
                        values_to_set=[intervention["value"]],
        ...
        if "biolord" in methods_to_eval:
            ...
            try:
                ...
                pred_adata_biolord = []
                for intervention in interventions:
                    pred_adata_biolord.append(
                        biolord_model.predict_intervention(
                            adata_inter.copy(),
                            hold_out_label="dose",
                            concepts_to_flip=[intervention["concept"]],
                            values_to_set=[intervention["value"]],
    ...
    if not summary_data:
        ...
        summary_df.to_csv(
            f"./rebuttal_benchmark_liver_results_summary_baselines_{methods_str}_seed_{RANDOM_SEED}.csv",
            index=False,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cinemaot", action="store_true", help="Evaluate CinemaOT baseline"
    )
    parser.add_argument("--scgen", action="store_true", help="Evaluate scGEN baseline")
    parser.add_argument(
        "--cemvae",
        action="store_true",
        help="Evaluate CEVAE baseline (not implemented)",
    )
    parser.add_argument(
        "--biolord",
        action="store_true",
        help="Evaluate Biolord baseline (not implemented)",
    )
