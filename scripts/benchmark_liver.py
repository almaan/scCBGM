...
def train_cbgm(
    adata_train, obsm_key="X_pca", hard_concept_key=None, soft_concept_key=None
):
    """
    ...
    if hard_concept_key:
        hard_concepts_data = (
            adata_train.obsm[hard_concept_key].to_numpy().astype(np.float32)
        )
    ...
    if soft_concept_key:
        soft_concepts_data = (
            adata_train.obsm[soft_concept_key].to_numpy().astype(np.float32)
        )
...
def pred_cbgm(
    ...
    adata_inter,
    obsm_key="X_pca",
    hard_concept_key=None,
    soft_concept_key=None,
    interventions=None,
):
    """
    ...
    x_intervene_on = torch.tensor(
        adata_inter.obsm[obsm_key] if obsm_key != "X" else adata_inter.X,
        dtype=torch.float32,
    )
    ...
    device = "cuda"
    with torch.no_grad():
        inter_preds_dict = model.intervene(
...
def get_learned_concepts(
    ...
    with torch.no_grad():
        ...
        if scCBGM_model.use_hard_concepts:
            scCBGM_concepts_known_hard = (
                scCBGM_model.cb_hard_layers(enc["mu"]).cpu().numpy()
            )
            scCBGM_concepts_known_hard_df = pd.DataFrame(
                scCBGM_concepts_known_hard,
                index=adata_full.obs.index,
                columns=adata_full.obsm[hard_concept_key].columns,
            )
        if scCBGM_model.use_soft_concepts:
            scCBGM_concepts_known_soft = (
                scCBGM_model.cb_soft_layers(enc["mu"]).cpu().numpy()
            )
            scCBGM_concepts_known_soft_df = pd.DataFrame(
                scCBGM_concepts_known_soft,
                index=adata_full.obs.index,
        ...
        scCBGM_concepts_unknown_df = pd.DataFrame(
            scCBGM_concepts_unknown,
            index=adata_full.obs.index,
    ...
    if scCBGM_model.use_hard_concepts and scCBGM_model.use_soft_concepts:
        scCBGM_concepts = pd.concat(
            [
                scCBGM_concepts_known_hard_df,
                scCBGM_concepts_known_soft_df,
                scCBGM_concepts_unknown_df,
            ],
            axis=1,
        )
    elif scCBGM_model.use_hard_concepts:
        scCBGM_concepts = pd.concat(
            [scCBGM_concepts_known_hard_df, scCBGM_concepts_unknown_df], axis=1
        )
    elif scCBGM_model.use_soft_concepts:
        scCBGM_concepts = pd.concat(
            [scCBGM_concepts_known_soft_df, scCBGM_concepts_unknown_df], axis=1
        )
    else:
...
def train_cb_fm(adata_train, concept_key="scCBGM_concepts", obsm_key="X_pca"):
    ...
    fm_model.train_loop(
        ...
        concepts=torch.from_numpy(
            adata_train.obsm[concept_key].to_numpy().astype(np.float32)
        ),
        num_epochs=NUM_EPOCHS,
        batch_size=128,
        lr=3e-4,
...
def pred_raw_fm(
    ...
    if edit:
        ...
        inter_preds = model.decode(
            h=torch.from_numpy(edit_concepts).to("cuda"), n_steps=1000, w_cfg=1.0
        )
...
def train_cvae(
    ...
    """
    Trains and returns the scCVAE model, supporting separate hard and soft concepts.
    """
    ...
    if hard_concept_key:
        ...
        concept_parts.append(hard_concepts_data)

    if soft_concept_key:
        soft_concepts_data = (
            adata_train.obsm[soft_concept_key].to_numpy().astype(np.float32)
        )
        concept_parts.append(soft_concepts_data)

    # Concatenate all concepts
    all_concepts = np.concatenate(concept_parts, axis=1)
    n_concepts = all_concepts.shape[1]
    ...
    # --- Model Instantiation ---
    model = clab.models.sc_cvae.scCVAE(config)

    # --- Training Loop ---
    model.train_loop(
        data=torch.from_numpy(data_matrix.astype(np.float32)),
        concepts=torch.from_numpy(all_concepts),
        num_epochs=NUM_EPOCHS,
        batch_size=128,
        lr=3e-4,
    )
...
def pred_cvae(
    ...
    Args:
        model: The trained scCVAE model.
        adata_inter: AnnData object with data to perform intervention on.
        obsm_key: Key in adata_inter.obsm to use as input, or 'X' for adata_inter.X.
        hard_concept_key: Key in adata_inter.obsm for the hard concepts DataFrame.
        soft_concept_key: Key in adata_inter.obsm for the soft concepts DataFrame.
        edit_concept: The name of the concept to edit.
        edit_value: The new value for the concept.
    """
    ...
    if hard_concept_key:
        hard_concepts_df = adata_inter.obsm[hard_concept_key]
        concept_parts.append(hard_concepts_df.to_numpy(dtype=np.float32))
        n_hard = hard_concepts_df.shape[1]

    if soft_concept_key:
        soft_concepts_df = adata_inter.obsm[soft_concept_key]
        concept_parts.append(soft_concepts_df.to_numpy(dtype=np.float32))
    ...
    c_intervene_on_numpy = np.concatenate(concept_parts, axis=1)
    ...
    # --- Find which concept to edit ---
    if edit_concept is not None and edit_value is not None:
        found = False
        if hard_concepts_df is not None and edit_concept in hard_concepts_df.columns:
            col_idx = hard_concepts_df.columns.get_loc(edit_concept)
            concepts_dec[:, col_idx] = edit_value
            found = True
            print(
                f"Intervening on HARD concept '{edit_concept}' (index {col_idx}) -> {edit_value}"
            )

        elif soft_concepts_df is not None and edit_concept in soft_concepts_df.columns:
            col_idx = soft_concepts_df.columns.get_loc(edit_concept) + n_hard
            concepts_dec[:, col_idx] = edit_value
            found = True
            print(
                f"Intervening on SOFT concept '{edit_concept}' (index {col_idx}) -> {edit_value}"
            )

        if not found:
            print(
                f"Warning: Concept '{edit_concept}' not found in provided keys. No intervention applied."
            )
...
def get_cvae_latents(
    ...
    # --- 2. Get Conditional Data 'concepts' (both hard and soft) ---
    concept_parts = []
    if hard_concept_key:
        hard_concepts_data = (
            adata_full.obsm[hard_concept_key].to_numpy().astype(np.float32)
        )
        concept_parts.append(hard_concepts_data)

    if soft_concept_key:
        soft_concepts_data = (
            adata_full.obsm[soft_concept_key].to_numpy().astype(np.float32)
        )
        concept_parts.append(soft_concepts_data)

    # Concatenate all concepts
    all_concepts_numpy = np.concatenate(concept_parts, axis=1)
...
def train_cvae_fm(
    adata_train, concept_key="scCVAE_latent_and_concepts", obsm_key="X_pca"
):
    """Trains and returns the cVAE-FM model using CVAE latents."""
    print("Training cVAE-FM model...")
...
def pred_cvae_fm(
    model,
    adata_inter,
    concept_key="scCVAE_latent_and_concepts",
    obsm_key="X_pca",
    hard_concept_key=None,
    soft_concept_key=None,
    edit_concept=None,
    ...
    Args:
        model: The trained cVAE-FM model.
        adata_inter: AnnData object with intervention data.
        concept_key: Key in adata_inter.obsm for the latent+concepts representation.
        obsm_key: Key for data matrix in obsm.
        hard_concept_key: Key for hard concepts (used to find concept index).
        soft_concept_key: Key for soft concepts (used to find concept index).
        edit_concept: Name of the concept to edit.
        edit_value: New value for the concept.
        edit: If True, use edit mode; if False, use decode mode.
    """
    ...
    # Get the initial latent+concepts representation
    ...
    if edit_concept is not None and edit_value is not None:
        ...
        # Build the hard and soft concept dataframes to find the index
        hard_concepts_df, soft_concepts_df = None, None
        n_hard = 0
        ...
        if hard_concepts_df is not None and edit_concept in hard_concepts_df.columns:
            col_idx = hard_concepts_df.columns.get_loc(edit_concept)
            # Offset by latent dimension to get into concept part
            edit_concepts[:, latent_dim + col_idx] = edit_value
            found = True
            print(
                f"Intervening on HARD concept '{edit_concept}' (index {latent_dim + col_idx}) -> {edit_value}"
            )
            ...
            found = True
            print(
                f"Intervening on SOFT concept '{edit_concept}' (index {latent_dim + col_idx}) -> {edit_value}"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark different methods on liver dose response data"
    )
    parser.add_argument(
        "--method",
        type=str,
        default="all",
        choices=["all", "cbgm", "cb-fm", "raw-fm", "cvae", "cvae-fm"],
        help="Method to evaluate. Options: all, cbgm, cb-fm, raw-fm, cvae, cvae-fm (default: all)",
    )
    ...
    print(f"Evaluating method(s): {EVAL_METHOD}")
    ...
    if not DATA_PREPROCESSED:
        ...
        adata.obs["cell_types_L2"] = adata.obs["cell_type__ontology_label"].astype(
            "category"
        )
        adata.obs["cell_types_L1"] = (
            adata.obs["cell_type__ontology_label"]
            .map(cell_type_mapping)
            .astype("category")
        )

        adata.obsm[HARD_CONCEPT_KEY] = pd.get_dummies(
            adata.obs[["cell_types_L1"]]
        ).astype(np.float32)
        adata.obsm[SOFT_CONCEPT_KEY] = pd.DataFrame(
            zero_one_norm(np.log(adata.obs["dose"].astype(float).values + 1)).reshape(
                -1, 1
            ),
        ...
        adata.obsm["concepts"] = pd.concat(
            [adata.obsm[HARD_CONCEPT_KEY], adata.obsm[SOFT_CONCEPT_KEY]], axis=1
        )

        adata.obs["L1_dose"] = [
            l1_ctype + "_" + dose
            for l1_ctype, dose in zip(adata.obs["cell_types_L1"], adata.obs["dose"])
        ]
        adata.obs["L2_dose"] = [
            l2_ctype + "_" + dose
            for l2_ctype, dose in zip(adata.obs["cell_types_L2"], adata.obs["dose"])
        ]
    ...
    for cell_type in base_cell_types:
        ...
        if not all(label in all_dose_labels for label in required_labels):
            print(
                f"Skipping '{cell_type}': Does not have all required dose labels (1.0, 3.0, 10.0)."
            )
            continue
        ...
        if len(adata_inter.X) < 300 or len(adata_test.X) < 300:
            print(
                f"Skipping '{cell_type}': Intervention set ({len(adata_inter.X)}) or test set ({len(adata_test.X)}) is smaller than 1000 cells."
            )
            continue
        ...
        for label in hold_out_labels:
            target_dose_value = (
                adata_test[adata_test.obs["L2_dose"] == label]
                .obsm[SOFT_CONCEPT_KEY]["dose"]
                .astype(float)
                .mean()
            )
            interventions.append(
                {"concept": "dose", "value": target_dose_value, "label": label}
            )
        ...
        pred_adata_cvae = []
        pred_adata_cvae_fm_edit = []
        pred_adata_cvae_fm_guid = []
        ...
        if EVAL_METHOD in [
            ...
            "cbgm",
            "cb-fm",
        ]:  # cb-fm needs cbgm for learned concepts
            cbgm_model = train_cbgm(
                adata_train.copy(),
                hard_concept_key=HARD_CONCEPT_KEY,
                soft_concept_key=SOFT_CONCEPT_KEY,
            )

            if EVAL_METHOD in ["all", "cbgm"]:
                for intervention in interventions:
                    pred_adata_cbgm.append(
                        pred_cbgm(
                            cbgm_model,
                            adata_inter.copy(),
                            hard_concept_key=HARD_CONCEPT_KEY,
                            soft_concept_key=SOFT_CONCEPT_KEY,
                            interventions={
                                intervention["concept"]: intervention["value"]
                            },
                        )
                    )

        # --- Method 2: CB-FM with Learned Concepts ---
        if EVAL_METHOD in ["all", "cb-fm"]:
            adata_with_concepts = get_learned_concepts(
                cbgm_model,
                adata.copy(),
                hard_concept_key=HARD_CONCEPT_KEY,
                soft_concept_key=SOFT_CONCEPT_KEY,
            )
            adata_train.obsm["scCBGM_concepts"] = adata_with_concepts[
                adata_train.obs.index
            ].obsm["scCBGM_concepts"]
            adata_inter.obsm["scCBGM_concepts"] = adata_with_concepts[
                adata_inter.obs.index
            ].obsm["scCBGM_concepts"]

            cb_fm_model = train_cb_fm(
                adata_train.copy(), concept_key="scCBGM_concepts", obsm_key=OBSM_KEY
            )

            for intervention in interventions:
                pred_adata_fm_edit.append(
                    pred_cb_fm(
                        cb_fm_model,
                        adata_inter.copy(),
                        concept_key="scCBGM_concepts",
        ...
        if EVAL_METHOD in ["all", "raw-fm"]:
            fm_raw_model = train_raw_fm(
                adata_train.copy(), concept_key="concepts", obsm_key=OBSM_KEY
            )

            for intervention in interventions:
                pred_adata_raw_fm_edit.append(
                    pred_raw_fm(
                        fm_raw_model,
                        adata_inter.copy(),
                        concept_key="concepts",
        ...
        # --- Method 4: scCVAE ---
        if EVAL_METHOD in ["all", "cvae", "cvae-fm"]:  # cvae-fm needs cvae for latents
            ...
            if EVAL_METHOD in ["all", "cvae"]:
                for intervention in interventions:
                    pred_adata_cvae.append(
                        pred_cvae(
                            cvae_model,
                            adata_inter.copy(),
                            obsm_key=OBSM_KEY,
        ...
        # --- Method 5: cVAE-FM with CVAE Latents ---
        if EVAL_METHOD in ["all", "cvae-fm"]:
            # Generate CVAE latents for all data
            adata_with_cvae_latents = get_cvae_latents(
                cvae_model,
                adata.copy(),
                obsm_key=OBSM_KEY,
                hard_concept_key=HARD_CONCEPT_KEY,
            ...
            adata_train.obsm["scCVAE_latent_and_concepts"] = adata_with_cvae_latents[
                adata_train.obs.index
            ].obsm["scCVAE_latent_and_concepts"]
            adata_inter.obsm["scCVAE_latent_and_concepts"] = adata_with_cvae_latents[
                adata_inter.obs.index
            ].obsm["scCVAE_latent_and_concepts"]
            adata_inter.obsm["scCVAE_latent"] = adata_with_cvae_latents[
                adata_inter.obs.index
            ].obsm["scCVAE_latent"]

            # Train the cVAE-FM model
            cvae_fm_model = train_cvae_fm(
                adata_train.copy(),
                concept_key="scCVAE_latent_and_concepts",
                obsm_key=OBSM_KEY,
            )
            ...
            for intervention in interventions:
                pred_adata_cvae_fm_edit.append(
                    pred_cvae_fm(
                        cvae_fm_model,
                        adata_inter.copy(),
                        concept_key="scCVAE_latent_and_concepts",
                ...
                pred_adata_cvae_fm_guid.append(
                    pred_cvae_fm(
                        cvae_fm_model,
                        adata_inter.copy(),
                        concept_key="scCVAE_latent_and_concepts",
        ...
        for i, intervention in enumerate(interventions):
            ...
            intervention_adata = adata_test[
                adata_test.obs["L2_dose"] == intervention_label
            ]

            # Build predictions dictionary based on evaluated methods
            predictions_for_ivn = {}
            if EVAL_METHOD in ["all", "cbgm"] and len(pred_adata_cbgm) > 0:
                predictions_for_ivn["scCBGM"] = pred_adata_cbgm[i]
            if EVAL_METHOD in ["all", "cb-fm"] and len(pred_adata_fm_edit) > 0:
                predictions_for_ivn["CB-FM (edit)"] = pred_adata_fm_edit[i]
                predictions_for_ivn["CB-FM (guided)"] = pred_adata_fm_guid[i]
            if EVAL_METHOD in ["all", "raw-fm"] and len(pred_adata_raw_fm_edit) > 0:
                predictions_for_ivn["Raw-FM (edit)"] = pred_adata_raw_fm_edit[i]
                predictions_for_ivn["Raw-FM (guided)"] = pred_adata_raw_fm_guid[i]
            if EVAL_METHOD in ["all", "cvae"] and len(pred_adata_cvae) > 0:
                predictions_for_ivn["scCVAE"] = pred_adata_cvae[i]
            if EVAL_METHOD in ["all", "cvae-fm"] and len(pred_adata_cvae_fm_edit) > 0:
                predictions_for_ivn["cVAE-FM (edit)"] = pred_adata_cvae_fm_edit[i]
                predictions_for_ivn["cVAE-FM (guided)"] = pred_adata_cvae_fm_guid[i]
            ...
            w2_scores = {}
            pre_computed_w2_train = -1
            fid_scores = {}
            pre_computed_frechet_train = -1
            sd_scores = {}
            pre_computed_sd_train = -1
            for name, pred_adata in predictions_for_ivn.items():
                ...
                if pre_computed_w2_train < 0:
                    val = clab.evaluation.interventions.evaluate_intervention_w2_with_target(
                        x_train=adata_train.obsm[OBSM_KEY],
                        x_ivn=pred_adata.obsm[OBSM_KEY],
                        x_target=intervention_adata.obsm[OBSM_KEY],
                        labels_train=adata_train.obs["L2_dose"].values,
                    )
                    pre_computed_w2_train = val["pre_computed_w2_train"]
                else:
                    val = clab.evaluation.interventions.evaluate_intervention_w2_with_target(
                        x_train=adata_train.obsm[OBSM_KEY],
                        x_ivn=pred_adata.obsm[OBSM_KEY],
                        x_target=intervention_adata.obsm[OBSM_KEY],
                        labels_train=adata_train.obs["L2_dose"].values,
                        pre_computed_w2_train=pre_computed_w2_train,
                    )
                w2_ratio = val["w2_ratio"]
                w2_scores[name] = w2_ratio

                if pre_computed_frechet_train < 0:
                    val = clab.evaluation.interventions.evaluate_intervention_frechet_with_target(
                        x_train=adata_train.obsm[OBSM_KEY],
                        x_ivn=pred_adata.obsm[OBSM_KEY],
                        x_target=intervention_adata.obsm[OBSM_KEY],
                        labels_train=adata_train.obs["L2_dose"].values,
                    )
                    pre_computed_frechet_train = val["pre_computed_frechet_train"]
                else:
                    val = clab.evaluation.interventions.evaluate_intervention_frechet_with_target(
                        x_train=adata_train.obsm[OBSM_KEY],
                        x_ivn=pred_adata.obsm[OBSM_KEY],
                        x_target=intervention_adata.obsm[OBSM_KEY],
                        labels_train=adata_train.obs["L2_dose"].values,
                        pre_computed_frechet_train=pre_computed_frechet_train,
                    )
                fid_ratio = val["frechet_ratio"]
                fid_scores[name] = fid_ratio

                if pre_computed_sd_train < 0:
                    val = clab.evaluation.interventions.evaluate_intervention_w2_with_target(
                        x_train=adata_train.obsm[OBSM_KEY],
                        x_ivn=pred_adata.obsm[OBSM_KEY],
                        x_target=intervention_adata.obsm[OBSM_KEY],
                    ...
                    pre_computed_sd_train = val["pre_computed_w2_train"]
                else:
                    val = clab.evaluation.interventions.evaluate_intervention_w2_with_target(
                        x_train=adata_train.obsm[OBSM_KEY],
                        x_ivn=pred_adata.obsm[OBSM_KEY],
                        x_target=intervention_adata.obsm[OBSM_KEY],
                        labels_train=adata_train.obs["L2_dose"].values,
                        pre_computed_w2_train=pre_computed_sd_train,
                        sinkhorn_reg=0.1,
                    )
                sd_ratio = val["w2_ratio"]
                sd_scores[name] = sd_ratio

            cell_type_results[intervention_label] = {
                "mmd": mmd_scores,
                "w2": w2_scores,
                "frechet": fid_scores,
                "sinkhorn": sd_scores,
    ...
    for cell_type, interventions_dict in all_results.items():
        for intervention_label, models_dict in interventions_dict.items():
            for k_metric, models_res in models_dict.items():
                row = {
                    "cell_type": cell_type,
                    "intervention_target": intervention_label,
                    "metric": k_metric,
                }
    ...
    if not summary_data:
        ...
        summary_df.to_csv(
            f"./rebuttal_benchmark_liver_results_summary_seed_{RANDOM_SEED}_method_{EVAL_METHOD}.csv",
            index=False,
        )
        print(
            f"\nSummary results saved to './rebuttal_benchmark_liver_results_summary_seed_{RANDOM_SEED}_method_{EVAL_METHOD}.csv'"
        )
