...
class CppAuc:
    ...
    def __init__(self):
        self._handle = ctypes.CDLL(
            os.path.dirname(os.path.realpath(__file__)) + "/cpp_auc.so"
        )
    ...
    def roc_auc_score(
        self, y_true: np.array, y_score: np.array, sample_weight: np.array = None
    ) -> float:
        ...
        result = self._handle.cpp_auc_ext(
            y_score, y_true, n, sample_weight, n_sample_weights
        )
        return result
