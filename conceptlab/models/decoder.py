...
class FlowDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [FlowMLPBlock(emb_dim, dropout) for _ in range(n_layers)]
        )
        self.output_layer = nn.Sequential(
            nn.LayerNorm(emb_dim), nn.Linear(emb_dim, self.x_dim)
        )
    ...
    def forward(
        ...
        if c is not None:
            if self.c_embedder is None:
                raise ValueError(
                    "Conditional tensor 'c' was provided, but the model was initialized with c_dim=0."
                )
    ...
    def integrate(
        ...
        if c is not None and self.c_embedder is None:
            raise ValueError(
                "Conditional tensor 'c' was provided, but the model is unconditional."
            )
        if x0 is not None and c is not None and x0.size(0) != c.size(0):
            raise ValueError(
                f"Batch size mismatch: x0 has batch size {x0.size(0)} but c has {c.size(0)}."
            )
