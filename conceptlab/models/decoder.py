...
class SkipDecoderBlock2(nn.Module):
    def __init__(
        self,
        input_dim: int,
        n_unknown: int,
        ...
        **kwargs,
        ...
        layers = []
        layers_dim = [self.n_unknown] + self.hidden_dim
    ...
    def forward(self, input_concept, unknown, **kwargs):
        h = self.decoder_layers(unknown)
        h, _ = self.final((h, input_concept))
        return dict(x_pred=h)
