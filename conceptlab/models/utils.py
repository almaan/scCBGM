...
def optimal_transport_coupling(x0, x1):
    ...
    actual_pairing_cost = torch.sum(
        M[torch.arange(b, device=device), assignment_indices]
    )

    assert torch.allclose(
        actual_pairing_cost / b, ot_plan_cost
    ), f"Cost mismatch: {(actual_pairing_cost / b).item():.4f} != {ot_plan_cost.item():.4f}"

    cost_improvement = torch.sum(
        M[torch.arange(b, device=device), assignment_indices]
    ) / torch.sum(torch.diag(M))
