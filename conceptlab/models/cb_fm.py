...
class CB_FM:
    ...
    def __init__(
        ...
        if self.c_dim == 0:
            print(
                "Warning: All concept dimensions are 0. Model will be purely unconditional."
            )
    ...
    def _get_train_step_data(
        self, x1: torch.Tensor, ot: bool
    ...
    def train(
        ...
        if concepts_known is None and concepts_unknown is None and self.c_dim > 0:
            raise ValueError(
                "Model has c_dim > 0, but no concepts were provided for training."
            )
        ...
        def unpack_batch(batch):
            ...
            c_known_batch = (
                batch[1].to(self.device) if concepts_known is not None else None
            )
            c_unknown_batch = (
                batch[-1].to(self.device) if concepts_unknown is not None else None
            )
        ...
        for epoch in pbar:
            ...
            for batch in dataloader:
                ...
                if self.c_unknown_dim > 0:
                    mask = (
                        torch.rand(c_unknown_batch.shape[0], device=self.device)
                        > p_drop
                    ).float()
                    c_parts_masked.append(c_unknown_batch * mask[:, None])

                c_total = (
                    torch.cat(c_parts_masked, dim=1)
                    if c_parts_masked
                    else torch.zeros(xt.shape[0], 0, device=self.device)
                )
            ...
            pbar.set_description(
                ...
                + (f", OT Improv: {avg_ot_improv:.4f}" if ot else "")
            )

    def _prepare_all_concept_vectors(
        self,
        ...
        all_concepts = [
            ...
            for c in [
                concepts_known,
                concepts_unknown,
                negative_concepts_known,
                negative_concepts_unknown,
            ]
            if c is not None
        ]
        ...
        if len(batch_sizes) > 1:
            raise ValueError(
                f"Inconsistent batch sizes in 2D concept tensors: {batch_sizes}"
            )
        if batch_sizes:
        ...
        def broadcast(tensor: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
            if tensor is None:
                return None
            if tensor.dim() == 1:
                return tensor.repeat(num_samples, 1).to(self.device)
            return tensor.to(self.device)

        return (
            broadcast(concepts_known),
            broadcast(concepts_unknown),
            broadcast(negative_concepts_known),
            broadcast(negative_concepts_unknown),
            num_samples,
        )
    ...
    def sample(
        ...
        c_cond = (
            torch.cat(c_cond_parts, dim=1)
            if c_cond_parts
            else torch.zeros(num_samples, 0, device=self.device)
        )
        ...
        print(
            f"Generating {num_samples} samples with CFG, w_pos={w_pos}"
            + (f" and w_neg={w_neg}" if c_neg is not None else "")
        )
    ...
    def _prepare_edit_tensors(
        ...
        if xt.dim() == 1:
            cf_batch_sizes = {
                c.shape[0]
                for c in [cf_concepts_known, cf_concepts_unknown]
                if c is not None and c.dim() == 2
            }
            if len(cf_batch_sizes) > 1:
                raise ValueError(
                    f"Inconsistent batch sizes in counterfactual concepts: {cf_batch_sizes}"
                )
            ...
            raise ValueError(
                f"x_orig must be a 1D or 2D tensor, but got {xt.dim()} dimensions."
            )

        all_concepts = [
            orig_concepts_known,
            orig_concepts_unknown,
            cf_concepts_known,
            cf_concepts_unknown,
        ]
        processed_concepts = []
        for concept in all_concepts:
            ...
            if concept.dim() == 1:
                ...
                raise ValueError(
                    f"Concept tensors must be 1D or 2D, but got {concept.dim()} dimensions."
                )
        ...
        orig_concept_parts = [
            c for c in [orig_concepts_known, orig_concepts_unknown] if c is not None
        ]
        c_orig = (
            torch.cat(orig_concept_parts, dim=1)
            if orig_concept_parts
            else torch.zeros(num_samples, 0, device=self.device)
        )

        cf_concept_parts = [
            c for c in [cf_concepts_known, cf_concepts_unknown] if c is not None
        ]
        c_cf = (
            torch.cat(cf_concept_parts, dim=1)
            if cf_concept_parts
            else torch.zeros(num_samples, 0, device=self.device)
        )
    ...
    def edit(
        ...
        cf_concepts_unknown: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """
        ...
        print(
            f"Generating {num_samples} counterfactual sample(s) from {num_distinct_x} original data point(s) with {num_distinct_cf} counterfactual condition(s)."
        )
        if w_cfg != 1.0:
        ...
        if abs(t_edit - t_edit_snapped) > 1e-6:
            print(
                f"Warning: t_edit snapped from {t_edit:.4f} to {t_edit_snapped:.4f} to align with discrete timesteps."
            )
        ...
        for t_step in tqdm(
            range(timesteps, t_edit_step, -1), desc="Encoding (Backward)", ncols=88
        ):
        ...
        print(
            f"Decoding: Integrating forward to t=1.0 with new condition and noise {noise_scale}..."
        )
        # Loop from t=t_edit up to t=1-dt
        for t_step in tqdm(
            range(t_edit_step, timesteps), desc="Decoding (Forward)", ncols=88
        ):
            t_val = t_step / timesteps
            t = torch.full((num_samples,), t_val, device=self.device)
...
class CBMFM_MetaTrainer:
    ...
    def get_learned_concepts(self, adata_full):
        ...
        with torch.no_grad():
            ...
            adata_full.obsm["scCBGM_concepts_known"] = (
                self.scCBGM_model.model.cb_concepts_layers(enc["mu"]).cpu().numpy()
            )
            adata_full.obsm["scCBGM_concepts_unknown"] = (
                self.scCBGM_model.model.cb_unk_layers(enc["mu"]).cpu().numpy()
            )

        adata_full.obsm["scCBGM_concepts"] = np.concatenate(
            [
                adata_full.obsm["scCBGM_concepts_known"],
                adata_full.obsm["scCBGM_concepts_unknown"],
            ],
    ...
    def train(self, adata_train):
        ...
        if self.obsm_key != "X":
            ...
            if self.z_score:
                data_matrix = (
                    data_matrix - adata_train.var["mean"].to_numpy()[None, :]
                ) / adata_train.var["std"].to_numpy()[
                    None, :
        ...
        if self.raw:
            ...
            mod_cfg["n_concepts"] = (
                adata_train.obsm[self.concept_key].to_numpy().shape[1]
            )
            ...
            self.fm_model.train_loop(
                data=torch.from_numpy(data_matrix.astype(np.float32)),
                concepts=torch.from_numpy(
                    adata_train.obsm["scCBGM_concepts"].astype(np.float32)
                ),
                num_epochs=self.num_epochs,
                batch_size=self.batch_size,
                lr=self.lr,
    ...
    def predict_intervention(
        ...
        if not self.raw:
            ...
            c_known_inter = torch.from_numpy(
                adata_inter.obsm["scCBGM_concepts_known"].astype(np.float32)
            )
            c_unknown_inter = torch.from_numpy(
                adata_inter.obsm["scCBGM_concepts_unknown"].astype(np.float32)
            )

            inter_concepts_known = c_known_inter.clone()

            inter_concepts_known[:, concept_to_intervene_idx] = (
                1
                - torch.Tensor(adata_inter.obsm[self.concept_key].values)[
                    :, concept_to_intervene_idx
                ]
        ...
        if self.obsm_key != "X":
            ...
            if self.z_score:
                x_inter = (
                    x_inter - adata_inter.var["mean"].to_numpy()[None, :]
                ) / adata_inter.var["std"].to_numpy()[None, :]
        ...
        if self.raw:
            ...
            edit_concepts[:, concept_to_intervene_idx] = (
                1 - edit_concepts[:, concept_to_intervene_idx]
            )  # Set stim concept to the opposite of the observed value.
        ...
        # Using CBM concepts
            ...
            edit_concepts = np.concatenate(
                [inter_concepts_known, c_unknown_inter], axis=1
            )
            ...
            if self.edit:
                inter_preds = self.fm_model.edit(
                    ...
                    c_prime=torch.from_numpy(edit_concepts.astype(np.float32)).to(
                        "cuda"
                    ),
                    t_edit=0.0,
                    n_steps=1000,
        ...
        with torch.no_grad():
            ...
            if self.scCBGM_model.model.use_hard_concepts:
                scCBGM_concepts_known_hard = (
                    self.scCBGM_model.model.cb_hard_layers(enc["mu"]).cpu().numpy()
                )
            ...
            if self.scCBGM_model.model.use_soft_concepts:
                scCBGM_concepts_known_soft = (
                    self.scCBGM_model.model.cb_soft_layers(enc["mu"]).cpu().numpy()
                )
            ...
            scCBGM_concepts_unknown_df = pd.DataFrame(
                ...
                index=adata_full.obs.index,
                columns=[
                    f"unknown_{i}" for i in range(scCBGM_concepts_unknown.shape[1])
                ],
            )

        if (
            self.scCBGM_model.model.use_hard_concepts
            and self.scCBGM_model.model.use_soft_concepts
        ):
    ...
    def train(self, adata_train):
        ...
        if self.raw:
            ...
            self.fm_model.train_loop(
                data=torch.from_numpy(data_matrix.astype(np.float32)),
                concepts=torch.from_numpy(
                    adata_train.obsm["scCBGM_concepts"].to_numpy().astype(np.float32)
                ),
                num_epochs=self.num_epochs,
                batch_size=self.batch_size,
                lr=self.lr,
...
class ConceptFlow_MetaTrainer:
    ...
    def predict_intervention(
        self, adata_inter, hold_out_label, concepts_to_flip, values_to_set: float = 1.0
    ):
        """Performs intervention using a trained learned-concept CB-FM model.
        Concepts_to_flip: List of binary concepts to flip during intervention."""
        ...
        inter_concepts[:, concept_to_intervene_idx] = (
            values_to_set
            if isinstance(values_to_set, (int, float))
            else values_to_set[0]
        )
        ...
        with torch.no_grad():
            inter_preds = self.fm_model.intervene(
