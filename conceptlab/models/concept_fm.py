...
class Concept_FM(nn.Module, ABC):
        ...
        **kwargs,
        ...
        if self.p_uncond > 0:
            self.condition_embedder = ConditionEmbedding(
                c_dim=self.n_concepts + self.n_unknown, emb_dim=self.hidden_dim
            )
    ...
    def cbm(self, z, concepts=None, mask=None, intervene=False, **kwargs):
        ...
        output_dict.update(
            {"input_concept": input_concept, "pred_concept": known_concepts, "h": h}
        )
        return output_dict
    ...
    def _hard_concept_loss(self, pred_concept, concepts, **kwargs):
        return self.n_concepts * F.binary_cross_entropy(
            pred_concept, concepts, reduction="mean"
        )

    def loss_function(
        self, v_pred, v, concepts, pred_concept, mu, logvar, unknown=None, **kwargs
    ):
        loss_dict = {}
        ...
        if self.use_orthogonality_loss:
            if unknown is not None:
                ...
                print(
                    "Warning: use_orthogonality_loss is True, but 'unknown' tensor was not provided."
                )
    ...
    def train_loop(
        ...
        for epoch in pbar:
            ...
            for x_batch, concepts_batch in data_loader:
                ...
                h_emb = self.condition_embedder(h)
                is_unconditional = (
                    torch.rand(x_batch.shape[0], device=device) < self.p_uncond
                )
