...
class CinemaOT:
    ...
    def predict_intervention(self, adata_inter, hold_out_label, concepts_to_flip, values_to_set = None):
        ...
        adata_cat.obs["split_col"] = ["train"] * len(self.adata_train) + ["int"] * len(adata_inter)
        ...
        adata_t_not = adata_cat[(adata_cat.obs["treated"]==1) | (adata_cat.obs["non_treated"]==1)].copy()

        assert (adata_t_not.obs["treated"].values == 1-adata_t_not.obs["non_treated"].values).all()
