...
class Biolord:
    ...
    def train(self,adata_train):
        ...
        concepts_df = adata_train_.obsm[self.concept_key].copy().rename(columns = concept_names_map)
    ...
    def predict_intervention(self, adata_inter, hold_out_label, concepts_to_flip, values_to_set = None):
        ...
        observed_concept_names = ["concept_"+ c for c in observed_concept_names_]
        concept_names_map = dict(zip(observed_concept_names_, observed_concept_names))
        concepts_df = adata_inter_.obsm[self.concept_key].copy().rename(columns = concept_names_map)
        ...
        adata_preds = self.model.compute_prediction_adata(self.adata_train, adata_inter_, target_attributes=renamed_concepts_to_flip)
