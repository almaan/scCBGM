...
class Mixed_CBM_MetaTrainer:
    ...
    def train(self, adata_train):
        ...
        if self.hard_concept_key:
            hard_concepts_data = adata_train.obsm[self.hard_concept_key].to_numpy().astype(np.float32)
        ...
        if self.soft_concept_key:
            soft_concepts_data = adata_train.obsm[self.soft_concept_key].to_numpy().astype(np.float32)
    ...
    def predict_intervention(self, adata_inter, hold_out_label, concepts_to_flip, values_to_set = None):
        ...
        for concept_name, new_value in zip(concepts_to_flip, values_to_set):
            ...
            if not found:
                raise ValueError(f"Warning: Concept '{concept_name}' not found in provided keys. Ignoring.")

        # --- Run intervention on the model ---
        device = 'cuda'
        with torch.no_grad():
            inter_preds_dict = self.model.intervene(x_intervene_on.to(device), mask=mask.to(device), concepts=inter_concepts.to(device))
